...
def parallelize_compute_X_tilde_and_counts(X, omega):

    n_workers = mp.cpu_count() - 4
    pool = Pool(n_workers)

    Xs = torch.split(X, n_workers)
    args = [  [X, omega] for X in Xs ]
    results = pool.starmap(compute_X_tilde_and_counts, args)

    final_Omega, final_cubes = defaultdict(list), Counter()

    keys = set(itertools.chain.from_iterable([set(result[0].keys()) for result in results]))
    final_Omega = {key: itertools.chain.from_iterable([Omega[key] for Omega, _  in results ]) for key in keys  }

    final_cubes = sum([cubes for _, cubes in results ], Counter())

    assert (set(final_Omega.keys()).difference(set(final_cubes.keys())) ) == set()

    # standard average
    X_tilde = stack([stack(list(value)).mean(axis=0) for key, value in final_Omega.items()])
    # for key, value in final_Omega.items():
        # final_Omega[key] = torch.stack(value).mean(axis=0)
        # value = list(value)
        # print("value", value)
        # print(final_Omega[key])

    return X_tilde, final_cubes
